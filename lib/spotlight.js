...
var configDefaults = {
      ...
      enforceHashtagMatches: true, // spotlight tweets with words/screennames matching user's tweeted hashtags, or only spotlight if they use the hashtag (#)
      recency: {
        hashtags: 10, // in days
    ...
    inspectFeed(userTweets, function(tweet){
      ...
      if (withinXDays(tweet.created_at, config.recency.hashtags) && tweet.entities.hashtags.length) {
        ...
        _.each(tweet.entities.hashtags, function(hashtagObj){
          var term = hashtagify(hashtagObj.text).toLowerCase();
          if (!smartListReasonsByTerm[term]) {
            smartListReasonsByTerm[term] = [];
            smartListReasonsByTerm[term].push('You used the hashtag '+hashtagify(hashtagObj.text)+' within the last '+config.recency.hashtags+' days');
          }
        });
      }
  ...
  filterTimeline: function (timeline, user, cookies) {
    ...
    _.each(timeline, function(tweet){
      _.each(tweet.entities.urls, function(urlObj){
        var link = '<a target="blank" href="'+urlObj.expanded_url+'">'+urlObj.display_url+'</a>';
    ...
    var prependTweets = [
      ...
      "Click the spotlight icon in the upper right hand corner of a tweet to see why it was spotlighted."
    ];
    
    var appendTweets = [
      "Thanks for using Twitter Timeline Spotlight.  If you enjoyed it, tweet about it!  I also welcome any feedback.<br>"+
      '<a href="https://twitter.com/share" class="twitter-share-button" data-text="I\'m using #TwitterTimelineSpotlight to turn down the noise on twitter & highlight the conversations I want to hear" data-size="large">Tweet</a>'+
      ...
      '<a href="/">Refresh</a> to see new tweets.'
    ];
