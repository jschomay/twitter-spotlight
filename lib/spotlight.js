  ...
  */
  makeSmartList: function (userTweets, mentions, favorites, followers, friends) {
    ...
    inspectFeed(userTweets, function(tweet){
      ...
      if (withinXDays(tweet.created_at, config.recency.usersYouInteractWith) && tweet.retweeted) {
        users.push(tweet.retweeted_status.user.screen_name); //original tweeter
      }
    });
    ...
    // TODO if desired - bring in retweets_of_me api data to get users who retweeted me
    ...
    var recentFollowers = _.chain(followers.users).filter(function(user) {return user.following;}).take(5).pluck('screen_name').value();
    users = users.concat(recentFollowers);

    // add last 3 users you followed
    var recentlyFollowed = _.chain(friends.users).filter(function(user) {return user.following;}).take(3).pluck('screen_name').value();
